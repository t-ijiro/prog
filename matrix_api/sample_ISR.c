...
extern const unsigned char arrow[4][8];
extern unsigned char vert_cnt;
extern unsigned char fore;
extern unsigned char back;
extern unsigned int  An_X, An_Y;
...
void Excep_CMT1_CMI1(void)
{
	...
	for(y = 0; y < 8; y++)
	{
		...
		if(arrow[dat][vert_cnt] & (1 << y))
		{
			c = (enum led_color)fore;
		}
		else
		{
			c = (enum led_color)back;
		}

		matrix_write(vert_cnt, y, c);
	}

    matrix_flush();

	rg_data = matrix_convert(vert_cnt);

	matrix_out(vert_cnt, rg_data);
}
